...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fast_staged_optimizer.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class FastStagedParameterOptimizer:
    """快速分阶段参数优化器"""
    ...
    def stage1_buy_threshold_test(self) -> Dict[str, Any]:
        ...
        for i, threshold in enumerate(buy_thresholds):
            ...
            logger.info(f"第一阶段测试 {i+1}/10: 买入阈值 {threshold} TAO")
            result = self._run_single_simulation(test_params)
            result['buy_threshold_price'] = threshold
            stage1_results.append(result)
            
        # 找到最优买入阈值
        best_result = max(stage1_results, key=lambda x: x.get('roi_3d', 0))
        ...
        logger.info(f"第一阶段最优ROI: {best_result.get('roi_3d', 0):.2f}%")
    ...
    def stage2_buy_step_test(self) -> Dict[str, Any]:
        ...
        for i, step in enumerate(buy_steps):
            test_params = {
                **fixed_params,
                'buy_step_size_tao': step
            }
            
            logger.info(f"第二阶段测试 {i+1}/9: 买入步长 {step} TAO")
            result = self._run_single_simulation(test_params)
            result['buy_step_size_tao'] = step
            stage2_results.append(result)
            
        # 找到最优买入步长
        best_result = max(stage2_results, key=lambda x: x.get('roi_3d', 0))
        ...
        logger.info(f"第二阶段最优ROI: {best_result.get('roi_3d', 0):.2f}%")
    ...
    def stage3_sell_strategy_test(self) -> Dict[str, Any]:
        ...
        fixed_params = {
            ...
            'simulation_days': 5,      # 5天测试周期，快速测试
            'enable_selling': True
        }
        
        # 测试变量：卖出条件
        sell_multipliers = [1.2, 1.5, 2.0, 2.5, 3.0]
        min_hold_days = [1, 2, 3, 4]  # 缩短持有天数以适应短周期测试
        ...
        for multiplier in sell_multipliers:
            for hold_days in min_hold_days:
                ...
                logger.info(f"第三阶段测试 {test_count}/{total_tests}: 卖出倍数 {multiplier}x, 最短持有 {hold_days}天")
                result = self._run_single_simulation(test_params)
        ...
        # 找到最优卖出策略
        best_result = max(stage3_results, key=lambda x: x.get('roi_5d', 0) / (x.get('payback_days', 999) + 1))
        ...
        logger.info(f"第三阶段最优ROI: {best_result.get('roi_5d', 0):.2f}%")
    ...
    def _run_single_simulation(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            config = {
                "config_version": "2.0",
                "simulation": {
                    "days": params.get('simulation_days', 3),
                    "blocks_per_day": 7200,
                    "tempo_blocks": 360,
                    "tao_per_block": "1.0"
                },
    ...
    def run_all_stages(self) -> Dict[str, Any]:
        ...
        final_report = {
            'optimization_summary': {
                ...
                'final_roi': stage3_result['optimal'].get('roi_5d', 0),
        ...
        logger.info(f"快速分阶段优化完成，总用时: {total_time:.2f}秒")
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        # 保存完整的JSON结果
        json_filename = f'fast_staged_optimization_results_{timestamp}.json'
        with open(json_filename, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False, default=str)
        
        # 额外保存CSV格式的详细数据便于分析
        csv_filename = f'fast_staged_optimization_data_{timestamp}.csv'
    ...
    def _save_csv_data(self, results: Dict[str, Any], filename: str):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                ...
                'sell_trigger_multiplier', 'min_hold_days', 'roi', 'final_value', 
                'total_investment', 'payback_days', 'max_drawdown', 'win_rate', 
            ...
            for stage_name, stage_data in results['stage_results'].items():
                ...
                for i, test_result in enumerate(stage_data['results']):
                    ...
                    row = {
                        ...
                        'simulation_days': test_result.get('config', {}).get('simulation', {}).get('days', '')
                    }
                    writer.writerow(row)

def main():
    """主函数"""
    logger.info("启动快速Bittensor分阶段优化引擎")
    
    optimizer = FastStagedParameterOptimizer()
    ...
    print(f"最优买入阈值: {results['optimization_summary']['optimal_parameters']['buy_threshold_price']} TAO")
    print(f"最优买入步长: {results['optimization_summary']['optimal_parameters']['buy_step_size_tao']} TAO")
