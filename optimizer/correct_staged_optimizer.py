...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('correct_staged_optimizer.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class CorrectStagedParameterOptimizer:
    """正确的分阶段参数优化器"""
    ...
    def stage1_buy_threshold_test(self) -> Dict[str, Any]:
        ...
        """
        logger.info("=== 开始第一阶段：买入阈值优化测试（30天）===")
        ...
        fixed_params = {
            ...
            'total_budget_tao': 1000   # 1000 TAO预算
        }
        ...
        for i, threshold in enumerate(buy_thresholds):
            ...
            logger.info(f"第一阶段测试 {i+1}/10: 买入阈值 {threshold} TAO (30天周期)")
    ...
    def stage2_buy_step_test(self) -> Dict[str, Any]:
        ...
        """
        logger.info("=== 开始第二阶段：买入步长优化测试（30天）===")
        ...
        for i, step in enumerate(buy_steps):
            ...
            logger.info(f"第二阶段测试 {i+1}/9: 买入步长 {step} TAO (30天周期)")
            result = self._run_single_simulation(test_params)
    ...
    def stage3_sell_strategy_test(self) -> Dict[str, Any]:
        ...
        第三阶段：卖出策略优化测试（60天周期）
        """
        logger.info("=== 开始第三阶段：卖出策略优化测试（60天）===")
    ...
    def run_all_stages(self) -> Dict[str, Any]:
        ...
        logger.info(f"正确分阶段优化完成，总用时: {total_time:.2f}秒")
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        # 保存完整的JSON结果
        json_filename = f'correct_staged_optimization_results_{timestamp}.json'
        with open(json_filename, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False, default=str)
        
        # 额外保存CSV格式的详细数据便于分析
        csv_filename = f'correct_staged_optimization_data_{timestamp}.csv'
    ...
    def _save_csv_data(self, results: Dict[str, Any], filename: str):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                ...
                'total_trades', 'total_tao_spent', 'total_dtao_purchased',
                'simulation_days'
            ]
            ...
            for stage_name, stage_data in results['stage_results'].items():
                ...
                for i, test_result in enumerate(stage_data['results']):
                    ...
                    row = {
                        ...
                        'total_tao_spent': test_result.get('total_tao_spent', ''),
                        'total_dtao_purchased': test_result.get('total_dtao_purchased', ''),
...
def main():
    """主函数"""
    logger.info("启动正确的Bittensor分阶段优化引擎")
    
    optimizer = CorrectStagedParameterOptimizer()
